...
CI 平台须可在内网首次离线搭建。Jenkins 离线部署（.deb + apt）：有网机 `fetch_offline.py` 下 jenkins/java 的 `.deb` + 插件到 `tools/ci/offline/`；内网 `deploy.py` 用 apt 装 `.deb`、放插件、渲染 JCasC、配 systemd（deb 自带 `jenkins.service` + `ci-webhook` 适配器）。Jenkins admin 密码经 `config.local.ini [secrets]` → systemd EnvironmentFile(0600) 注入，不入仓不落命令行（C-1）。见 `server/deploy/`、`docs/05_jenkins_design.md`、`docs/OFFLINE_DEPENDENCIES.md`。
...
Jenkins 离线件（jenkins/java 的 `.deb` + 插件）在**有网机**经 `server/deploy/fetch_offline.py` 下载到
`tools/ci/offline/`，随 FR-16 推送到远端 `[offline] deps_dir`。下载经环境变量 `HTTPS_PROXY`（**含明文密码，
...
|------|------|
| 许可证 | MIT（免费商用，NFR-6） |
| 自托管不外泄 | 支持；离线部署（jenkins/java 的 .deb + 插件 离线传入，apt 安装，NFR-1/FR-9） |
...
└── fetch_offline.py 下 jenkins/java 的 .deb+插件  ┌──────────────────────────────────────┐
...
的 `.deb`、放插件、渲染 JCasC、配 systemd（deb 自带 `jenkins.service` + `ci-webhook` 适配器）。日常构建触发走 HTTP
...
（jenkins/java 的 `.deb` + 插件）在**有网机**经 `fetch_offline.py` 下载到 `tools/ci/offline/`（走 `HTTPS_PROXY`
