...
| O-3 | 仿真并发限制 | ✅ 已确认 | **严格串行，License = 1**（Jenkins numExecutors=1） |
...
针对仿真软件不支持并行的约束，CI 须保证仿真步骤独占执行：同一时刻仅一个仿真任务运行（或不超过 License 实例数），其余任务排队等待，不得因抢占导致 License 冲突或工具状态污染。**实现（D-016 Jenkins）：Jenkins `numExecutors=1`——单节点同一时刻仅 1 个构建，天然串行 = License 数；多节点扩容时用 throttle-concurrents/lockable-resources 按类别限并发。** 见 `server/deploy/jenkins.yaml`（JCasC）。
...
CI 平台须可在内网首次离线搭建。Jenkins 离线部署：有网机 `fetch_offline.py` 下 WAR+插件(含依赖)+JDK21 打包；内网 `deploy.py` 解包、放插件、渲染 JCasC、装 systemd 服务（`ci-jenkins` + `ci-webhook` 适配器）。Jenkins admin 密码经 `config.local.ini [secrets]` → systemd EnvironmentFile(0600) 注入，不入仓不落命令行（C-1）。见 `server/deploy/`、`docs/05_jenkins_design.md`、`docs/OFFLINE_DEPENDENCIES.md`。

**FR-10 Jenkins 插件清单**

须固化 Jenkins 所需插件清单（`server/deploy/plugins.txt`，依赖由 plugin-manager 自动解析）：
`git`/`workflow-aggregator`(pipeline)/`configuration-as-code`(JCasC)/`job-dsl`/`throttle-concurrents`/
`mcp-server`。Jenkins 离线、`-Djenkins.install.runSetupWizard=false`，不对外联网上报/遥测（C-1）。
...
Jenkins 离线包（WAR+插件+JDK）在**有网机**经 `server/deploy/fetch_offline.py` 一键下载打包，再随 FR-16
推送到远端 `[offline] deps_dir`。下载经环境变量 `HTTPS_PROXY`（**含明文密码，严禁入仓**，仅置环境变量，
C-1）。版本号在 `fetch_offline.py` 顶部变量、清单见 `OFFLINE_DEPENDENCIES.md`。决策见 D-010。
...
webhook 触发的评测构建由 Jenkins 自带队列与执行器调度：`numExecutors=1` 串行（=License 数），同分支
...
| server·部署 | 服务器本地离线装 Jenkins（手动跑，需 root） | `server/deploy/`（deploy.py / fetch_offline.py / plugins.txt / jenkins.yaml / systemd/） |
...
|------|------|
| 许可证 | MIT（免费商用，NFR-6） |
| 自托管不外泄 | 支持；离线部署（WAR + 插件 + JDK 离线传入，NFR-1/FR-9） |
| 仿真串行控制 | `numExecutors=1`（单节点同一时刻 1 个构建 = License 数）；多节点用 throttle-concurrents/lockable-resources |
| 无 Docker 执行 | 内置节点直接执行 shell（NFR-4） |
| 配置方式 | Pipeline(Groovy) + JCasC 配置即代码（D-018，随配置版本化，NFR-5/NFR-7） |
| 插件安全 | 最小化插件清单 `plugins.txt`，跟踪 CVE |

> Jenkins 用 LTS WAR + 官方插件（MIT/宽松许可，NFR-6）；不启用任何对外联网上报/遥测（C-1）。
...
- **串行控制**：`numExecutors=1`——单节点同一时刻仅 1 个构建，天然串行 = License 数（D-003）；同分支 auto-cancel 由 job 的 `disableConcurrentBuilds(abortPrevious)` 保证。多节点扩容时用 throttle-concurrents/lockable-resources 按类别限并发。
...
└── fetch_offline.py 下 WAR+插件+JDK 打包         ┌──────────────────────────────────────┐
                                                 │ Jenkins（WAR + JCasC，HTTP 直连）       │
执行机（admin）                                  │   └ qsort-eval pipeline job（参数化）   │
├── connectivity.py（连通性自检）                │   numExecutors=1 串行 + auto-cancel     │
└── deploy_remote.py ─SSH/SCP 推码+离线包───────▶ │ 评测：limited_run 限制 + eval.py 功能+性能│
...
| Jenkins 插件供应链风险 | 最小化插件清单 `plugins.txt`、离线固定版本、跟踪 CVE |
...
| D-003 | 仿真严格串行（Jenkins numExecutors=1） | 仿真 License=1，不支持并行 |
...
**D-003 仿真严格串行**：仿真软件 License 单实例、不支持并行。Jenkins `numExecutors=1`——单节点
同一时刻仅 1 个构建，天然串行 = License 数；多节点扩容时用 throttle-concurrents/lockable-resources
按类别限并发，受真实 License 数约束。**贯穿全程不可动摇的硬约束。**
...
(2) **服务器本地部署（server/deploy）**：代码+离线包到位后在服务器上直接跑 `deploy.py`，解包装 Jenkins
（WAR+插件+JDK）、渲染 JCasC、起 systemd 服务（`ci-jenkins` + `ci-webhook` 适配器）。日常构建触发走 HTTP
...
**D-010 首次远端 bootstrap + 离线包获取**：内网新机无外网、不能 git clone / 在线装插件。故 Jenkins 离线包
（WAR+插件+JDK）在**有网机**经 `fetch_offline.py` 下载打包（走 `HTTPS_PROXY` 环境变量，明文密码不入仓），
再随代码经 SSH/SCP 推到远端 `[offline] deps_dir`。传输用 OpenSSH `ssh`/`scp`。
