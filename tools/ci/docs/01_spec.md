...
CI 平台须可在内网首次离线搭建。Jenkins 离线部署（.deb + apt）：有网机手动下 jenkins/java 的 `.deb`、`fetch_plugins.py`（curl + 公网 update-center.json 解依赖）下全部插件+依赖并打 tar.gz 到 `tools/ci/local/offline/`，经介质/手动拷到服务器；内网手动三步：`install.sh` apt 装 `.deb`、手动把插件 `.hpi` 拷进 `/var/lib/jenkins/plugins`、`gen_jenkins_yaml.py` 渲染 JCasC（经 UI「Configuration as Code → Apply」或拷到 `/var/lib/jenkins/jenkins.yaml` + systemd `CASC_JENKINS_CONFIG` 加载）。Jenkins admin 密码经 `config.local.ini [secrets]` 或 env 注入，不入仓不落命令行（C-1）。见 `server/deploy/`、`docs/05_jenkins_design.md`、根 `README.md`。

**FR-10 Jenkins 插件清单**

须固化 Jenkins 所需插件清单（`server/deploy/plugins.txt`）。**插件全部离线**（内网 Update Center 装不了）：
有网机 `fetch_plugins.py` 据清单用 curl + 公网 `update-center.json` 下全部插件 + 全部依赖（**纯 Python 解析整棵依赖树** + sha256 校验 + MANIFEST 闭包自检，不漏），
打 tar.gz 经介质拷到服务器，手动把 `.hpi` 拷进 `/var/lib/jenkins/plugins`。核心：`git`/`workflow-aggregator`(pipeline)/
`configuration-as-code`(JCasC)/`job-dsl`/`throttle-concurrents`(D-003)，另含凭证/清理/UI/邮件等；完整清单以 `server/deploy/plugins.txt` 为准。
Jenkins 启动加三个 `-D`：`jenkins.install.runSetupWizard=false`（跳过 setup wizard）、
`hudson.model.UpdateCenter.never=true`（关 Update Center 自动检查）、`hudson.model.UsageStatistics.disabled=true`
（关匿名使用统计上报）——内网无外联，调了只超时刷日志，全程不对外联网（C-1）。

**FR-11 离线 / 单端口 / 一键部署**

CI 系统须适配受限内网：(a) 全部脚本为 python3 3.8 标准库实现，零第三方依赖；(b) 依赖离线传入（不 git clone），见根 `README.md`；(c) 离线件 + 代码经介质/手动拷到服务器，在服务器上手动装（`install.sh` 装 .deb + 拷插件 + `gen_jenkins_yaml.py` 渲染 JCasC）；日常触发走 HTTP，不依赖常驻隧道；(d) 所有可变参数归一到 `config.ini`，敏感项归一到 `config.local.ini`（不入仓）。决策依据见 D-004、D-008。

**FR-12 开发端 CI 操作 MCP**

须提供 MCP 作为开发端查询 CI 的接口：查构建状态、拉错误日志。**实现（D-016）：用 Jenkins 官方 `mcp-server` 插件（装上即暴露 job/build 为 MCP 工具），不再自写 MCP。** 凭证（admin + API token）经环境变量/客户端配置，不入仓（C-1）。触发构建用分支源插件（FR-14），不在此。MCP 与被测代码无关。决策依据见 D-006。
...
须支持 git push 触发构建。**实现（D-020）：用内网自研【分支源插件】+ 组织文件夹——插件扫内源 git、
自动发现带 `Jenkinsfile` 的仓/分支并建 job、push 即触发；不再用自研 webhook 适配器。** 加项目=仓里放
`Jenkinsfile`，Jenkins 零改动/零重启。说明见 `docs/05_jenkins_design.md`、`docs/Jenkinsfile.example`。

**FR-15 端口固定与白名单**

Jenkins 端口由 `config.ini [jenkins] http_port` 固定，须经白名单（80-90/443/8080-8090）校验（`gen_jenkins_yaml.py`）。

**FR-17 离线依赖获取（手动 / 自动）**

Jenkins 离线件在**有网机**产出：jenkins/java 的 `.deb` 手动下；插件经 `local/admin/fetch_plugins.py`（curl + 公网
update-center.json 解依赖）下全部插件+依赖并打 `jenkins-plugins.tar.gz`，落 `tools/ci/local/offline/`，经介质/手动
拷到服务器 `[offline] deps_dir`。下载代理在 `config.ini [proxy]`（**含明文密码的放 `config.local.ini`，严禁入仓**，
C-1）。jenkins/java 的 .deb 版本/URL 与插件清单见根 `README.md`。
...
git push 触发的评测构建由 Jenkins 自带队列与执行器调度：`numExecutors`=总并行（可配），单仿真器由 throttle 类别限其 license，同分支
...
| FR-14 | git push 触发构建（分支源插件 + 组织文件夹，D-020） | Accepted |
...
| 角色 | 何时 / 何地执行 | 仓库位置 |
|------|----------------|---------|
| 共享层 | 配置 / 公共库 / 文档（两边都读） | `ci_config.py`、`config.ini`、`config.local.ini`(不入仓)、`docs/` |
| server·部署 | 服务器本地离线装 Jenkins（手动三步，需 root） | `server/deploy/`（install.sh / gen_jenkins_yaml.py / plugins.txt / jenkins.yaml） |
| server·运行时 | git push→分支源插件触发→跑各仓 Jenkinsfile（D-020） | `server/deploy/jenkins.yaml`(JCasC 组织文件夹)、各仓 `Jenkinsfile`、`server/harness/limited_run.py`(NFR-3)、`server/demo/qsort/eval.py` |
| local·首次 | 有网机：下插件+依赖打 tar.gz、手动下 .deb | `local/admin/fetch_plugins.py`、`local/offline/`（离线件暂存） |
...
`mcp-server` 插件的 MCP 端点（D-016，无自写代码）。触发构建用分支源插件（FR-14）。与被测代码无关。见第 8 节 D-006。
...
|------|------|
| 许可证 | MIT（免费商用，NFR-6） |
| 自托管不外泄 | 支持；离线部署（jenkins/java 的 .deb apt 安装；插件全离线公网下，NFR-1/FR-9） |
...
```
有网机（一次性，纯外网即可）                   CI 服务器（内网，无外网）
├── fetch_plugins.py：curl 下插件+依赖 →tar.gz   ┌──────────────────────────────────────┐
└── 手动下 jenkins/java 的 .deb                   │ Jenkins（.deb apt 装 + JCasC，HTTP 直连）│
        │                                        │   组织文件夹(分支源插件)自动发现带      │
        └─ 介质/手动拷 .deb + tar.gz + 代码 ─────▶ │   Jenkinsfile 的仓 → 建 job/触发        │
           服务器本地手动装(脚本3步) ──────────▶ │   numExecutors=N 并行 + throttle 限仿真  │
                                                 │ 评测：limited_run 限制 + 各仓 Jenkinsfile│
内源 git push ─▶ 分支源插件(Jenkins 内)─自动发现/触发─▶ 各仓 Jenkinsfile（编译/仿真）────────▶│
开发端 opencode ──MCP（mcp-server 插件）查 job/build──────────────────────▶ └──────────────┘
```

说明：离线件先在有网机产出（插件 `fetch_plugins.py` curl 下 + 打 tar.gz；.deb 手动下），经介质/手动拷到
服务器后本地手动装（`install.sh` 装 .deb + 拷插件 + `gen_jenkins_yaml.py` 渲染 JCasC，D-008）——下载机与服务器分属两网、不可 SSH 直达，故无 SSH 推送。触发由自研
**分支源插件**在 Jenkins 内直接收 git 事件完成（D-020），不再有独立适配器。
...
| Jenkins 插件供应链风险 | 公网下固定版本、离线传入、最小化清单 `plugins.txt`、跟踪 CVE |
...
| D-008 | 部署 = 服务器本地手动三步（`install.sh` 装 .deb + 拷插件 + `gen_jenkins_yaml.py` 渲染 JCasC）；Jenkins HTTP 直连 | 离线件就位后服务器本地手动装；端口 config 固定 + 白名单校验（gen） |
| D-009 | 凭证 / 密钥 / 代理明文密码不入仓 | 仅置环境变量或 config.local.ini（gitignore），严禁入仓与 git 历史 |
...
| D-019 | 离线交付：curl 取插件打 tar.gz + 手动下 .deb + 介质拷贝、服务器本地部署 | 下载机与服务器分属两网不可 SSH；curl+update-center 纯 Python 解依赖；不用 SSH 推送/plugin-cli/urllib |
| D-020 | 触发用自研分支源插件 + 组织文件夹（不自建 webhook 适配器） | 内网已有该插件，Jenkins 内直接发现/触发；流水线随各仓 Jenkinsfile，加项目零重启 |
...
**D-008 服务器本地手动部署**：离线件（.deb + 插件 + 代码）就位后在服务器上手动三步：① `install.sh` apt 装
jenkins/java 的 `.deb`；② 手动把插件 `.hpi` 拷进 `/var/lib/jenkins/plugins`；③ `gen_jenkins_yaml.py` 渲染 JCasC →
`jenkins.rendered.yaml`，经 UI「Configuration as Code → Apply」或拷到 `/var/lib/jenkins/jenkins.yaml`（+ systemd
`CASC_JENKINS_CONFIG`）加载。日常触发走 HTTP（FR-14），不依赖常驻隧道；Jenkins HTTP 同机直达；端口由
`config.ini [jenkins] http_port` 固定并经白名单（80-90/443/8080-8090）校验（`gen_jenkins_yaml.py`）。

**D-009 凭证 / 密钥 / 代理明文密码不入仓**：Jenkins admin 密码、HTTP(S) 代理的明文密码（`user:pass@proxy`）
一律不写入仓库与 git 历史，只经环境变量或 `config.local.ini` 注入（`.gitignore` 的 `*.local.ini` 已忽略）。
`ci_config.load()` 自动叠加 `config.local.ini` 覆盖，`secret()` 优先取环境变量；admin 密码运行时经 env 或手写的
systemd EnvironmentFile（0600）注入 Jenkins，不入仓、不落命令行。
...
**不自建 webhook 适配器**。每个被测项目仓放根目录 `Jenkinsfile`（编译/仿真 + `throttle([...])` 引用 license
类别，模板 `docs/Jenkinsfile.example`），**加项目=放 Jenkinsfile，Jenkins 零改动/零重启**；license 池（仿真器
+ 预留通用坑位 c1/c2/c3）在中央 `jenkins.yaml` 定义。
...
2. 在内网起 Jenkins，跑通完整链路 e2e（git push→分支源插件→Jenkins→Jenkinsfile→limited_run+eval）；重点验 JCasC/组织文件夹加载与 mcp-server 插件端点。
