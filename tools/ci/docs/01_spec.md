...
CI 平台须可在内网首次离线搭建。Jenkins 离线部署（.deb + apt）：有网机手动下 jenkins/java 的 `.deb`、`fetch_plugins.py`（curl + 公网 update-center.json 解依赖）下全部插件+依赖并打 tar.gz 到 `tools/ci/local/offline/`，经介质/手动拷到服务器；内网 `deploy.py` 用 apt 装 `.deb`、把插件 `.hpi` 拷进 `/var/lib/jenkins/plugins`、渲染 JCasC、配 systemd（deb 自带 `jenkins.service`）。Jenkins admin 密码经 `config.local.ini [secrets]` → systemd EnvironmentFile(0600) 注入，不入仓不落命令行（C-1）。见 `server/deploy/`、`docs/05_jenkins_design.md`、`docs/quick_deploy.md`。
...
打 tar.gz 经介质拷到服务器，`deploy.py` 把 `.hpi` 拷进 `/var/lib/jenkins/plugins`。核心：`git`/`workflow-aggregator`(pipeline)/
...
CI 系统须适配受限内网：(a) 全部脚本为 python3 3.8 标准库实现，零第三方依赖；(b) 依赖离线传入（不 git clone），见 `docs/quick_deploy.md`；(c) 离线件 + 代码经介质/手动拷到服务器，在服务器上一键跑 `server/deploy/deploy.py`；日常触发走 HTTP，不依赖常驻隧道；(d) 所有可变参数归一到 `config.ini`，敏感项归一到 `config.local.ini`（不入仓）。决策依据见 D-004、D-008。
...
Jenkins 端口由 `config.ini [jenkins] http_port` 固定，须经白名单（80-90/443/8080-8090）校验（`deploy.py check`）。
...
C-1）。jenkins/java 的 .deb 版本/URL 与插件清单见 `docs/quick_deploy.md`。
...
└── 手动下 jenkins/java 的 .deb                   │ Jenkins（.deb apt 装 + JCasC，HTTP 直连）│
        ...
        └─ 介质/手动拷 .deb + tar.gz + 代码 ─────▶ │   Jenkinsfile 的仓 → 建 job/触发        │
           服务器本地跑 deploy.py all ──────────▶ │   numExecutors=N 并行 + throttle 限仿真  │
...
服务器后本地直跑 `deploy.py`（D-008）——下载机与服务器分属两网、不可 SSH 直达，故无 SSH 推送。触发由自研
...
| D-008 | 部署 = 服务器本地一键 `deploy.py`；Jenkins HTTP 直连 | 离线件就位后服务器本地直跑；端口 config 固定 + 白名单校验 |
...
**D-008 服务器本地一键部署**：离线件（.deb + 插件 + 代码）就位后在服务器上直接跑 `deploy.py`：apt 装
jenkins/java 的 `.deb`、把插件 `.hpi` 拷进 `/var/lib/jenkins/plugins`、渲染 JCasC、配 systemd（deb 自带
`jenkins.service`）。日常触发走 HTTP（FR-14），不依赖常驻隧道；Jenkins HTTP 同机直达；端口由
`config.ini [jenkins] http_port` 固定并经白名单（80-90/443/8080-8090）校验（deploy.py check）。
...
`ci_config.load()` 自动叠加 `config.local.ini` 覆盖，`secret()` 优先取环境变量；部署时 deploy.py 把密钥写入
systemd EnvironmentFile（0600）供服务读取。
