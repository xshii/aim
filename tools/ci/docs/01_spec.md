...
CI 平台须可在内网首次离线搭建。Jenkins 离线部署（.deb + apt）：有网机 `fetch_offline.py` 下 jenkins/java 的 `.deb`、`fetch_plugins.py` 从公网下全部插件+依赖到 `tools/ci/local/offline/`；内网 `deploy.py` 用 apt 装 `.deb`、把插件 `.jpi` 拷进 `/var/lib/jenkins/plugins`、渲染 JCasC、配 systemd（deb 自带 `jenkins.service` + `ci-webhook` 适配器）。Jenkins admin 密码经 `config.local.ini [secrets]` → systemd EnvironmentFile(0600) 注入，不入仓不落命令行（C-1）。见 `server/deploy/`、`docs/05_jenkins_design.md`、`docs/OFFLINE_DEPENDENCIES.md`。

**FR-10 Jenkins 插件清单**

须固化 Jenkins 所需插件清单（`server/deploy/plugins.txt`）。**插件全部离线**（内网 Update Center 装不了）：
有网机 `fetch_plugins.py` 据清单用 plugin-cli 从**公网**下全部插件 + 全部依赖（**递归解析整棵依赖树**，不漏），
随 offline 传，`deploy.py` 把 `.jpi` 拷进 `/var/lib/jenkins/plugins`。核心：`git`/`workflow-aggregator`(pipeline)/
...
| local·首次 | 客户端：下离线件 + 远端 bootstrap（SSH/SCP） | `local/admin/`（fetch_offline.py / fetch_plugins.py / deploy_remote.py / connectivity.py）、`local/offline/`（离线件暂存） |
...
└── fetch_offline + fetch_plugins 下 .deb + 插件   ┌──────────────────────────────────────┐
...
的 `.deb`、把离线插件 `.jpi` 拷进 `/var/lib/jenkins/plugins`、渲染 JCasC、配 systemd（deb 自带 `jenkins.service` + `ci-webhook` 适配器）。日常构建触发走 HTTP
...
（jenkins/java 的 `.deb` + 全部插件 `.jpi`）在**有网机**经 `fetch_offline.py`/`fetch_plugins.py` 下载到 `tools/ci/local/offline/`（走 `HTTPS_PROXY`
