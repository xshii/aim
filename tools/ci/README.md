...
└── local/                  ← 在客户端 / 执行机上执行
    ├── admin/    fetch_offline.py（有网机下离线件）/ deploy_remote.py / connectivity.py
    ├── offline/  离线件暂存（jenkins/java .deb + plugin-cli jar；大文件 .gitignore，仅占位入仓）
...
# Java：把 JDK/JRE 21 的 .deb 也放进 tools/ci/local/offline/（插件由服务器从内源 UC 装）
```
...
```bash
cd /opt/ci
sudo python3 server/deploy/deploy.py all   # 自检 → apt 装 .deb + 从内源 UC 装插件 + JCasC → systemd(jenkins+ci-webhook)
```

## 离线依赖

见 `docs/OFFLINE_DEPENDENCIES.md`：有网机 `fetch_offline.py` 下 jenkins `.deb` + plugin-cli jar 到
`tools/ci/local/offline/`，Java 21 的 `.deb` 一并放入。**插件由服务器 `deploy.py` 从内源 Update Center 装**
（`config.ini [jenkins] update_center_url`，按 `server/deploy/plugins.txt`）。版本/URL 在 `config.ini [fetch]`。
