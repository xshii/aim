...
├── server/                 ← 在 CI 服务器上执行
│   ├── deploy/   deploy.py / plugins.txt / jenkins.yaml(JCasC) / systemd/
...
└── local/                  ← 在客户端 / 执行机上执行
    ├── admin/    fetch_offline.py / fetch_plugins.py（有网机下离线件）/ deploy_remote.py / connectivity.py
    ├── offline/  离线件暂存（jenkins/java .deb + plugins/*.jpi；大文件 .gitignore，仅占位入仓）
    └── mcp/      opencode.json.example   （指向 Jenkins 官方 mcp-server 插件）
```

| 角色 | 谁/何时 | 入口 |
|------|---------|------|
| **有网机·一次性** | 产出离线件（jenkins .deb + plugin-cli jar + java .deb） | `python3 local/admin/fetch_offline.py` |
...
python3 local/admin/fetch_offline.py     # 下 jenkins .deb + plugin-cli jar 到 tools/ci/local/offline/
python3 local/admin/fetch_plugins.py     # 据 plugins.txt 从公网下全部插件 + 全部依赖（递归解依赖）
# Java：把 JDK/JRE 21 的 .deb 也放进 tools/ci/local/offline/
```
...
```bash
cd /opt/ci
sudo python3 server/deploy/deploy.py all   # 自检 → apt 装 .deb + 拷离线插件进默认路径 + JCasC → systemd(jenkins+ci-webhook)
```

## 离线依赖

见 `docs/OFFLINE_DEPENDENCIES.md`：有网机 `fetch_offline.py` 下 jenkins/java 的 `.deb`、`fetch_plugins.py` 据
`plugins.txt` 从**公网**下全部插件 + 全部依赖（plugin-cli 递归解依赖）到 `tools/ci/local/offline/`。
**插件全离线**（内网 Update Center 装不了），`deploy.py` 把 `.jpi` 拷进 Jenkins 默认插件路径 `/var/lib/jenkins/plugins`。
...
- **CI 框架 Jenkins**（D-016）：标准特性开箱（pipeline-as-code、auto-cancel、Web UI、官方 MCP 插件）；
  .deb + apt 离线安装（jenkins/java 的 .deb；插件全离线 .jpi 拷入默认路径；JCasC 配置即代码 D-018）。
